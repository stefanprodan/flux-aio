...
#HelmReleaseSpec: {
	...
	// Helm 3 behavior),
	// 'combined' (hooks and templates sent together, Helm 4 default),
	// and 'separate'
	// (hooks and templates sent in separate streams, Helm 4.2
	// opt-in).
	// Defaults to 'combined', or 'nohooks' when the UseHelm3Defaults
	...
	// this HelmRelease.
	upgrade?: {
		// ChartNameChangeStrategy defines the strategy to use when a Helm
		// chart name changes.
		// Valid values are 'Reinstall' or 'InPlaceUpdate'. Defaults to
		// 'Reinstall' if omitted.
		//
	...
	valuesFrom?: [...{
		...
		// Literal marks this ValuesReference as a literal value. When set
		// in
		// combination with TargetPath, the referenced value is merged at
		// the target
		// path without interpreting Helm's `--set` syntax (commas,
		// brackets, dots,
		// equal signs, etc.), mirroring the behavior of `helm
		// --set-literal`. This
		// is the only safe way to inject arbitrary file content (config
		// files, JSON
		// blobs, multi-line strings containing special characters)
		// through
		// `valuesFrom`. Has no effect when TargetPath is empty: in that
		// mode the
